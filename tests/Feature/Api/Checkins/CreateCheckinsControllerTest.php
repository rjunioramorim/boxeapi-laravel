...
test('usuário não pode realizar checkin se a aula já tiver sido realizada', function () {
    ...
    Carbon::setTestNow();
    $response->assertStatus(422);
    $response->assertJson(['message' => 'Aula não está mais disponível, tente outro horário']);
});
...
test('usuário não pode realizar checkin se a aula já tiver com limite completo', function () {
    ...
    Carbon::setTestNow();
    $response->assertStatus(422);
    $response->assertJson(['message' => 'Aula está com limite completo, tente outro horário']);
});
