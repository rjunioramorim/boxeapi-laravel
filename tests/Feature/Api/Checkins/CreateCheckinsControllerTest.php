...
test('usuário não pode realizar checkin se a aula já tiver com limite completo', function () {
    ...
    Carbon::setTestNow();
    $response->assertStatus(422);
    $response->assertJson(['message' => 'Ops... Essa aula não tem mais vagas, tente outro horário']);
});
...
test('usuário não pode realizar checkin na mesma aula', function () {
    ...
    Carbon::setTestNow();
    $response->assertStatus(422);
    $response->assertJson(['message' => 'Você já tem uma aula agendada para esse horário']);
});
